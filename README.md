...
脚本每次运行会尝试下载最新 IP66 MMDB 数据库（`https://downloads.ip66.dev/db/ip66.mmdb`），用于校验 DoH 返回 IPv4 的国家和 ASN 运营商信息。国内 IPv4 会先筛选 `CN + 对应运营商 ASN`，再把能通过明确省份/城市 ASN 判断的结果作为优先候选；如果优先候选不足，会继续切换备用 DoH 查询，最后再用 `CN + 运营商` 的降权候选补齐。IPv6 不做 IP66 国家/地区校验，直接保留 DoH 返回结果；境外线路只查询 IPv4，拿到可用 A 记录后即停止继续探测。
...
| 分组 | IPv4 | IPv6 | 说明 |
|------|:----:|:----:|------|
| 国内三网默认 + 7 大区三网 | ✅ | ✅ | IPv4 使用 IP66 校验 CN+运营商并优先保留省份 ASN 命中结果；IPv6 不做 IP66 校验 |
| 全网默认 | ✅ | ✅ | 国内大区均衡双栈，每大区最多 2 个 IP |
| 境外默认/大洲/指定地域 | ✅ | ❌ | 仅查询 IPv4，拿到可用 A 记录后即停止继续探测 |
...
- 境外分组仅使用 IPv4；单个境外网段拿到可用 A 记录后会立即停止继续探测，减少 Google DoH 不可达时的等待
- DoH 公共接口有请求频率限制，脚本已内置 0.2s 查询间隔；国内网段优先使用腾讯云 `doh.pub`，境外网段优先使用 Google `dns.google`，失败时自动切换备用 DoH
- IP66 数据库每次运行会自动更新；如下载失败但本地已有 `ip66.mmdb`，会使用缓存；如未安装 `maxminddb` 或数据库不可用，则国内 IPv4 跳过 IP66 校验并退回原 DoH 查询逻辑；IPv6 始终不做 IP66 校验
